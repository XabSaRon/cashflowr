...
import type { IncomeFrequency, IncomeScope, IncomeRow } from "../api/incomes.service";
...
export function IncomeManageModal(props: {
  ...
  const recurrent = useMemo(() => rows.filter((r) => r.frequency !== "once"), [rows]);
  const oneOff = useMemo(() => rows.filter((r) => r.frequency === "once"), [rows]);
  ...
  const { ytdCents, runRateMonthlyCents, onceYtdCents, recurrentYtdCents } = useMemo(() => {
    const now = new Date();
    ...
    for (const r of rows) {
      ...
      if (r.frequency === "quarterly") runRateMonthly += Math.round(cents / 3);
  ...
  async function onDeleteConfirmed() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("incomes.manage.deleteError"));
    } finally {
      setBusyDelete(false);
    }
  }
  ...
  return (
    <div className="hmodal__backdrop hmodal__backdrop--income" role="dialog" aria-modal="true">
      <div className="hmodal__panel imgr__panel">
        <div className="imgr__head">
          <div>
            <div className="imgr__title">{t("dashboard.cards.incomes.title")}</div>
            <div className="imgr__sub">
              {usersLoading ? t("incomes.manage.loadingUsers") : t("incomes.manage.subtitle")}
            </div>
          </div>

          <button className="imgr__close" onClick={onClose} aria-label={t("common.close")}>
            ✕
          </button>
        </div>
        ...
        <div className="imgr__body">
          ...
          <section className="imgr__summary" aria-label={t("incomes.manage.summary", "Resumen")}>
            <div className="imgr__stat">
              <div className="imgr__statTop">
                <div className="imgr__statLabel">
                  {t("incomes.manage.ytd", "Ganado en {{year}}", { year })}
                </div>
                <div className="imgr__statHint">{t("incomes.manage.prorated", "YTD real")}</div>
              </div>

              <div className="imgr__statValue">{formatEUR(ytdCents, i18n.language)}</div>
              ...
              <div className="imgr__statBreakdown">
                ...
                <span>
                  {t("incomes.manage.once", "Puntual")}: {formatEUR(onceYtdCents, i18n.language)}
                </span>
              </div>
            </div>
            ...
            <div className="imgr__stat">
              <div className="imgr__statTop">
                <div className="imgr__statLabel">{t("incomes.manage.runRate", "Ingreso recurrente")}</div>
                <div className="imgr__statHint">{t("incomes.manage.perMonth", "/ mes")}</div>
              </div>

              <div className="imgr__statValue">{formatEUR(runRateMonthlyCents, i18n.language)}</div>

              <div className="imgr__statBreakdown">
                {t("incomes.manage.runRateHelp", "Mensual + (Trimestral/3) + (Anual/12)")}
              </div>
            </div>
          </section>
          ...
          <section className="imgr__section">
            <div className="imgr__sectionTop">
              <div className="imgr__sectionTitle">{t("incomes.manage.recurrentTitle")}</div>
              <span className="imgr__count">{recurrentSorted.length}</span>
            </div>

            {recurrentSorted.length === 0 ? (
              <div className="imgr__empty">{t("incomes.manage.emptyRecurrent")}</div>
            ) : (
                  ...
                  <div key={r.id} className="imgr__row">
                    <div className="imgr__left">
                      ...
                      <div className="imgr__meta">
                        <span className="imgr__chip">{freqLabel(r.frequency, (k) => t(k))}</span>

                        <span className={`imgr__chip ${r.scope}`}>
                          {scopeLabel(r.scope, (k) => t(k))}
                        </span>

                        <span className={`imgr__owner ${canEdit(r) ? "me" : "other"}`}>
                          {canEdit(r) ? (
                            <>
                              <span className="imgr__dot me" aria-hidden="true" />
                              <span className="imgr__ownerName">{t("incomes.manage.you")}</span>
                            </>
                          ) : (
                            <>
                              <span className="imgr__dot other" aria-hidden="true" />
                              <span className="imgr__ownerName">{ownerText(r.createdByUid)}</span>
                            </>
                          )}
                        </span>
                      </div>
                    ...
                    <div className="imgr__right">
                      <div className="imgr__amount">{formatEUR(r.amountCents, i18n.language)}</div>

                      {canEdit(r) ? (
                        <div className="imgr__actions" aria-label={t("incomes.manage.actions")}>
                          <button
                            type="button"
                        ...
                        <div className="imgr__lockedHint" title={t("incomes.manage.onlyYours")}>
          ...
          <section className="imgr__section">
            <div className="imgr__sectionTop">
              <div className="imgr__sectionTitle">{t("incomes.manage.oneOffTitle")}</div>
              <span className="imgr__count">{oneOff.length}</span>
            </div>

            {oneOff.length === 0 ? (
              <div className="imgr__empty">{t("incomes.manage.emptyOneOff")}</div>
            ) : (
              <div className="imgr__list">
                {oneOff.map((r) => {
                  const d = toDate(r.date);
                    ...
                    <div key={r.id} className="imgr__row">
                      <div className="imgr__left">
                        ...
                        <div className="imgr__meta">
                          ...
                          <span className="imgr__dateText">{formatDateShort(d, i18n.language)}</span>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          description={
            <div>
              {t("incomes.manage.deleteDesc")} <strong>{confirmDelete.source}</strong> ·{" "}
              <strong>{formatEUR(confirmDelete.amountCents, i18n.language)}</strong>
            </div>
          }
